...
- emits the async-only metrics (queue_depth on schedule, disk_seconds on finalize)
...
def test_async_save_reports_queue_depth_and_disk_latency(
    patched_checkpointer, tmp_path
):
    """async_save emits queue_depth on schedule and disk_seconds on finalize.

    Memory-ready latency is intentionally NOT emitted here — the trainer
    already records that under `timeperf/save` via `stats_tracker.record_timing`.
    Disk latency is the new metric that async save uniquely exposes.
    """
    mod, manager, queue = patched_checkpointer
    stats_scalar = sys.modules["areal.utils.stats_tracker"].scalar
    stats_scalar.reset_mock()

    # Sequence: 1) save schedules call_idx=42 -> emits queue_depth only
    #           2) next save reaps it (returns [42]) -> emits disk latency
    ...
    with (
        ...
        manager.save_checkpoint(str(tmp_path / "step0"))
        first_call_kwargs = stats_scalar.call_args_list[0].kwargs
        assert "ckpt/async_save_queue_depth" in first_call_kwargs
        # We deliberately do NOT emit a memory-ready metric — trainer owns that.
        assert "ckpt/async_save_memory_seconds" not in first_call_kwargs

        # On the next save, reap returns [42] -> disk latency should be emitted.
    ...
    assert "ckpt/async_save_disk_seconds" in all_keys
    # pending deque should now only hold the new call.
    assert len(manager._pending_save_starts) == 1
    assert manager._pending_save_starts[0][0] == 43
